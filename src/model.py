...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.enc2 = self.conv_bn_relu(
            8, 16, kernel_size=3, pool_kernel=2
        )  # 64x24x24
        self.enc3 = self.conv_bn_relu(
            16, 32, kernel_size=3, pool_kernel=1
        )  # 128x12x12
        self.enc4 = self.conv_bn_relu(
            32, 64, kernel_size=3, pool_kernel=3
        )  # 256x6x6
    ...
    def conv_bn_relu(self, in_ch, out_ch, kernel_size=3, pool_kernel=None):
        ...
        layers.append(
            nn.Conv2d(
                in_ch, out_ch, kernel_size, padding=(kernel_size - 1) // 2
            )
        )
...
class Discriminator(nn.Module):
    ...
    def conv_bn_relu(
        self, in_ch, out_ch, kernel_size=3, pool_kernel=None, reps=2
    ):
        layers = []
        for i in range(reps):
